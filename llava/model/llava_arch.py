...
class LlavaMetaModel(ABC):
    ...
    def encode_images(self, images, block_sizes: Optional[Optional[Tuple[int, ...]]] = None, is_depth: bool = False, use_depth_tower: bool = True):
        # Safety guard: ensure depth images have 3 channels before encoding
        if is_depth and isinstance(images, torch.Tensor):
            from llava.mm_utils import _ensure_depth_3channels
            images = _ensure_depth_3channels(images)
            # ✅ 检查点 2: Depth tensor 通道数验证
        ...
        if getattr(self.config, "dynamic_s2", False):
            # Final safety check before encoding in dynamic_s2 mode
            if is_depth and isinstance(images, torch.Tensor):
                from llava.mm_utils import _ensure_depth_3channels
                images = _ensure_depth_3channels(images)
            if is_depth and use_depth_tower:
            ...
            # Final safety guard before encoding - ALWAYS check depth tensors
            if is_depth and isinstance(images, torch.Tensor):
                from llava.mm_utils import _ensure_depth_3channels
                images = _ensure_depth_3channels(images)
                # Debug: verify channels after conversion
                if images.dim() == 4 and images.shape[1] != 3:
                    raise ValueError(f"Depth tensor should have 3 channels after conversion, but got {images.shape[1]} channels. Shape: {images.shape}")
            # Also check if depth tensor was accidentally passed to vision_tower
            if not is_depth and isinstance(images, torch.Tensor) and images.dim() == 4 and images.shape[1] == 1:
                # This might be a depth tensor passed incorrectly - convert to 3 channels
                from llava.mm_utils import _ensure_depth_3channels
                images = _ensure_depth_3channels(images)
            if is_depth and use_depth_tower:
                image_features = self.get_depth_tower()(images)
