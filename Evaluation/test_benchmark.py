...
def eval_task(task_name, model_name, model_generate_func, url, output_save_folder):
    ...
    if os.path.exists(output_path):
        print(f'{output_path} already exists, loading processed samples...')
        with open(output_path, 'r') as f:
            for line in f:
                try:
                    ...
                    pass
        print(f'Found {len(processed_ids)} already processed samples, continuing...')

    # 使用追加模式（如果文件已存在）或写入模式（如果文件不存在）
